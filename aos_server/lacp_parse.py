...
def parse_show_linkagg_detail(output: str, agg_id: str) -> Dict[str, Any]:
    """
    Parse 'show linkagg agg <id>' command output for detailed LAG information.
    
    Returns detailed information about a specific LAG including member ports.
    """
    result = {
        "agg_id": agg_id,
        "name": None,
    ...
    for line in lines:
        ...
        if "Operational State:" in line or "Oper State:" in line:
            match = re.search(r'State:\s*(up|down)', line, re.IGNORECASE)
            if match:
                result["oper_state"] = match.group(1).lower()
        
        # Parse member ports
        # Format: Port    Status      LACP Mode   Actor State
        # Example: 1/1/25  selected    active      aggregatable
        port_match = re.search(
            r'(\d+/\d+/\d+)\s+(selected|standby|unselected)\s+(active|passive|static)?\s*(\w+)?',
            line
        )
        if port_match:
            port, status, lacp_mode, actor_state = port_match.groups()
            
            member = {
                "port": port,
                "status": status.lower(),
                "lacp_mode": lacp_mode.lower() if lacp_mode else "static",
                "actor_state": actor_state.lower() if actor_state else None
            }
            
            result["members"].append(member)
    
    # Calculate bandwidth
    active_members = len([m for m in result["members"] if m["status"] == "selected"])
    if active_members > 0:
        # Assume 10G per port (can be enhanced with actual port speed)
        result["total_bandwidth"] = f"{active_members * 10}Gbps"
